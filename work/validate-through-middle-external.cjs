...
const OUTPUT_JSON = path.join(ROOT, "outputs", "middle-school-external-validation.json");
const OUTPUT_CSV = path.join(ROOT, "outputs", "middle-school-external-validation.csv");
const NAVER_CSV = path.join(ROOT, "outputs", "naver-manual-review-middle-school.csv");
const OUTPUT_MD = path.join(ROOT, "outputs", "middle-school-external-validation-report.md");
...
function hasMiddleLevel(levels) {
  return levels.some((level) => ["a1", "a2", "b1"].includes(level));
}
...
async function main() {
  ...
  const targetRows = targets.rows.filter((entry) => Number(entry.level) <= 3);
  ...
  const summary = {
    ...
    scope: "through-middle-school-candidate",
    apiMode,
    total: rows.length,
    ...
    oxfordA1A2B1Matches: rows.filter((entry) => hasMiddleLevel(entry.oxfordLevels)).length,
  ...
  const md = `# 중학생까지 사용 가능 수준 외부 기준 검증 결과

생성 시각: ${new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}

## 기준

- 검증 범위: 앱 내부 레벨 1~3 전체, 총 ${summary.total}개
- 한국어 뜻 최종 기준: 네이버 영어사전 수동 검수
- 자동 기준 1: Oxford 3000 and 5000 등재 여부, CEFR A1/A2/B1 우선 확인
...
| Oxford A1/A2/B1 등재 | ${summary.oxfordA1A2B1Matches} |
...
- 전체 자동검증 JSON: \`outputs/middle-school-external-validation.json\`
- 전체 자동검증 CSV: \`outputs/middle-school-external-validation.csv\`
- 네이버 수동 검수표: \`outputs/naver-manual-review-middle-school.csv\`
