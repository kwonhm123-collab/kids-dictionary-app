...
const OUTPUT_JSON = path.join(ROOT, "outputs", "elementary-external-validation.json");
const OUTPUT_CSV = path.join(ROOT, "outputs", "elementary-external-validation.csv");
const NAVER_CSV = path.join(ROOT, "outputs", "naver-manual-review-elementary.csv");
const OUTPUT_MD = path.join(ROOT, "outputs", "elementary-external-validation-report.md");
...
function partMatches(appPart, oxfordParts, apiParts) {
  ...
  return expected.some((item) => candidates.has(item)) ? "pass" : "review";
}
...
async function loadDictionaryApi(word) {
  ...
  if (cached && cached.status !== 429) return cached;
...
function statusFromChecks({ oxfordMatch, oxfordLevels, apiExists, partStatus }) {
  ...
  if (oxfordMatch && oxfordLevels.some((level) => level === "a1" || level === "a2")) return "auto-pass";
...
async function main() {
  ...
  const elementaryRows = targets.rows.filter((entry) => entry.audience === "elementary-candidate");
  const oxfordEntries = loadOxfordEntries();
  const rows = [];

  for (let index = 0; index < elementaryRows.length; index += 1) {
    const entry = elementaryRows[index];
    const word = normalizeWord(entry.word);
    const oxford = oxfordEntries.get(word) ?? [];
    const api = summarizeApi(await loadDictionaryApi(word));
    const oxfordParts = [...new Set(oxford.map((item) => item.part))];
    const oxfordLevels = [...new Set(oxford.flatMap((item) => item.levels))].sort();
    const partStatus = partMatches(entry.part, oxfordParts, api.parts);
  ...
  const summary = {
    ...
    scope: "elementary-candidate",
    total: rows.length,
    oxfordMatches: rows.filter((entry) => entry.oxfordMatch).length,
    oxfordA1A2Matches: rows.filter((entry) => entry.oxfordLevels.some((level) => level === "a1" || level === "a2")).length,
  ...
  const reviewRows = rows
    ...
    .map(
      (entry) =>
        `| ${entry.sequence} | ${entry.word} | ${entry.appKorean} | ${entry.appPart} | ${entry.oxfordMatch ? entry.oxfordLevels.join(", ") : "없음"} | ${entry.dictionaryApiExists ? "있음" : "없음"} | ${entry.partStatus} |`
    )
    .join("\n");

  const md = `# 초등 후보 336개 외부 기준 검증 결과
...
- 자동 기준 1: Oxford 3000 and 5000 등재 여부, CEFR A1/A2 우선 확인
...
| Oxford A1/A2 등재 | ${summary.oxfordA1A2Matches} |
...
| API 생략 | ${summary.apiSkipped} |

## 수동 확인 우선 목록

| 번호 | 단어 | 앱 뜻 | 앱 품사 | Oxford 레벨 | API | 품사 상태 |
|---:|---|---|---|---|---|---|
${reviewRows || "| - | - | - | - | - | - | - |"}

## 산출물

- 전체 자동검증 JSON: \`outputs/elementary-external-validation.json\`
- 전체 자동검증 CSV: \`outputs/elementary-external-validation.csv\`
- 네이버 수동 검수표: \`outputs/naver-manual-review-elementary.csv\`
